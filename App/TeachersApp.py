...
class TeacherTimetableApp(BaseTk):
    ...
    def create_menu_bar(self):
        ...
        tools_menu.add_separator()
        tools_menu.add_command(label="Set Supabase Settings", command=self.set_supabase_settings_dialog)
        tools_menu.add_command(label="Export Classroom Settings", command=self.export_classroom_settings)
        tools_menu.add_command(label="View Reached Logs", command=self.view_reached_logs)
        tools_menu.add_separator()
...
• Left-click row - Toggle select checkbox"""
    ...
    def set_supabase_settings_dialog(self):
        ...
        ttk.Label(dialog, text="Supabase URL:").grid(row=0, column=0, sticky='w')
        url_entry = ttk.Entry(dialog)
        url_entry.grid(row=0, column=1, sticky='ew')
        url_entry.insert(0, self.supabase_url)
        ttk.Label(dialog, text="Supabase Key:").grid(row=1, column=0, sticky='w')
        key_entry = ttk.Entry(dialog)
        key_entry.grid(row=1, column=1, sticky='ew')
        key_entry.insert(0, self.supabase_key)
        ttk.Label(dialog, text="Classroom Password:").grid(row=2, column=0, sticky='w')
        pass_entry = ttk.Entry(dialog)
        ...
        def save():
            ...
            if self.supabase_url and self.supabase_key and create_client:
                try:
                    ...
                    self.supabase = None
            dialog.destroy()
        ttk.Button(dialog, text="Save", command=save).grid(row=3, column=0, columnspan=2)
    ...
    def view_reached_logs(self):
        ...
        tree = ttk.Treeview(dialog, columns=("name", "class", "room", "period", "date", "timestamp", "source"), show="headings")
        tree.heading("name", text="Teacher")
        tree.heading("class", text="Class")
        tree.heading("room", text="Room")
        tree.heading("period", text="Period")
        tree.heading("date", "Date")
        tree.heading("timestamp", text="Timestamp")
        tree.heading("source", text="Source")
        for log in logs:
            tree.insert("", "end", values=(log['name'], log['class'], log['room'], log['period'], log['date'], log['reached_timestamp'], log['confirmation_source']))
