...
class TeacherTimetableApp(BaseTk):
    ...
    def create_menu_bar(self):
        ...
        file_menu.add_command(label="Print Multiple Timetables", command=self.print_multiple_timetables)
        file_menu.add_separator()
    ...
    def backup_database(self):
        ...
        """
        try:
            # Prepare default filename: database_bkp_DD-MM-YYYY_HH-MM-SS.db
            ts = _dt_now.now().strftime("%d-%m-%Y_%H-%M-%S")
    ...
    def ensure_credentials(self):
        if not self.load_setting('password_hash'):
            self.set_initial_password()

    def set_initial_password(self):
        dialog = tk.Toplevel(self)
        dialog.title("Set Initial Password")
        dialog.geometry("400x200")
        dialog.resizable(False, False)
        # Prevent closing the dialog without setting password
        ...
        dialog.bind('<Return>', lambda e: set_pass())
        ttk.Label(dialog, text="Username:", font=("Segoe UI", 16)).grid(row=0, column=0, padx=10, pady=5)
        username_var = tk.StringVar(value='admin')
        username_entry = ttk.Entry(dialog, textvariable=username_var, font=("Segoe UI", 16))
        username_entry.grid(row=0, column=1, padx=10, pady=5)
        ttk.Label(dialog, text="Password:", font=("Segoe UI", 16)).grid(row=1, column=0, padx=10, pady=5)
        password_var = tk.StringVar()
        password_entry = ttk.Entry(dialog, textvariable=password_var, show="*", font=("Segoe UI", 16))
        password_entry.grid(row=1, column=1, padx=10, pady=5)
        ttk.Label(dialog, text="Confirm Password:", font=("Segoe UI", 16)).grid(row=2, column=0, padx=10, pady=5)
        confirm_var = tk.StringVar()
        confirm_entry = ttk.Entry(dialog, textvariable=confirm_var, show="*", font=("Segoe UI", 16))
        confirm_entry.grid(row=2, column=1, padx=10, pady=5)
        def set_pass():
            if password_var.get() == confirm_var.get() and password_var.get():
                hash_val = hashlib.sha256(password_var.get().encode()).hexdigest()
                self.save_setting('username', username_var.get())
                self.save_setting('password_hash', hash_val)
                dialog.destroy()
            else:
                messagebox.showerror("Error", "Passwords do not match or are empty")
        ttk.Button(dialog, text="Set Password", command=set_pass, style="TButton").grid(row=3, column=0, columnspan=2, pady=10)
    ...
    def show_login(self):
        ...
        def login():
            ...
            stored_hash = self.load_setting('password_hash')
            stored_username = self.load_setting('username', 'admin')
            if username == stored_username and hashlib.sha256(password.encode()).hexdigest() == stored_hash:
                self.logged_in = True
                dialog.destroy()
            else:
                messagebox.showerror("Login Failed", "Invalid credentials")
        ttk.Button(dialog, text="Login", command=login, style="TButton").grid(row=2, column=0, columnspan=2, pady=10)
        self.wait_window(dialog)
        if not getattr(self, 'logged_in', False):
            self.quit()
    ...
    def change_password_dialog(self):
        ...
        dialog.bind('<Return>', lambda e: change_pass())

        main_frame = ttk.Frame(dialog)
        main_frame.pack(pady=15, padx=15, fill="both", expand=True)

        # Current Password
        row_frame1 = ttk.Frame(main_frame)
        row_frame1.pack(fill="x", pady=3)
        ttk.Label(row_frame1, text="Current Password:", font=("Segoe UI", 14)).pack(side="left")
        current_var = tk.StringVar()
        current_entry = ttk.Entry(row_frame1, textvariable=current_var, show="*", font=("Segoe UI", 14), width=25)
        current_entry.pack(side="right")

        # New Username
        row_frame2 = ttk.Frame(main_frame)
        row_frame2.pack(fill="x", pady=3)
        ttk.Label(row_frame2, text="New Username:", font=("Segoe UI", 14)).pack(side="left")
        new_username_var = tk.StringVar(value=self.load_setting('username', 'admin'))
        new_username_entry = ttk.Entry(row_frame2, textvariable=new_username_var, font=("Segoe UI", 14), width=25)
        new_username_entry.pack(side="right")

        # New Password
        row_frame3 = ttk.Frame(main_frame)
        row_frame3.pack(fill="x", pady=3)
        ttk.Label(row_frame3, text="New Password:", font=("Segoe UI", 14)).pack(side="left")
        new_password_var = tk.StringVar()
        new_password_entry = ttk.Entry(row_frame3, textvariable=new_password_var, show="*", font=("Segoe UI", 14), width=25)
        new_password_entry.pack(side="right")

        # Confirm New Password
        row_frame4 = ttk.Frame(main_frame)
        row_frame4.pack(fill="x", pady=3)
        ttk.Label(row_frame4, text="Confirm New Password:", font=("Segoe UI", 14)).pack(side="left")
        confirm_var = tk.StringVar()
        confirm_entry = ttk.Entry(row_frame4, textvariable=confirm_var, show="*", font=("Segoe UI", 14), width=25)
        confirm_entry.pack(side="right")

        # Button
        def change_pass():
            current_pass = current_var.get()
            stored_hash = self.load_setting('password_hash')
            if hashlib.sha256(current_pass.encode()).hexdigest() != stored_hash:
                messagebox.showerror("Error", "Current password is incorrect")
                return
            if new_password_var.get() != confirm_var.get() or not new_password_var.get():
                messagebox.showerror("Error", "New passwords do not match or are empty")
                return
            new_hash = hashlib.sha256(new_password_var.get().encode()).hexdigest()
            self.save_setting('username', new_username_var.get())
            self.save_setting('password_hash', new_hash)
            messagebox.showinfo("Success", "Password changed successfully")
            dialog.destroy()
        ttk.Button(main_frame, text="Change Password", command=change_pass, style="TButton").pack(pady=15)
    ...
    def create_basic_layout(self):
        ...
        # Auto-complete commands on Tab
        self.search_entry.bind('<Tab>', self.auto_complete_command)
    ...
    def auto_complete_command(self, event):
        """Auto-complete commands starting with # when Tab is pressed."""
        text = self.search_var.get().strip()
        if text.startswith('#'):
            prefix = text[1:].lower()
            commands = ['dup', 'absent', 'present', 'free', 'unmarked', 'class:']
            for cmd in commands:
                if cmd.startswith(prefix):
                    completed = '#' + cmd
                    self.search_var.set(completed)
                    self.search_entry.icursor(len(completed))  # Move cursor to end
                    break
        ...
        if tl == "#dup":
            try:
                ...
                if not duplicate_names:
                    # No duplicates found, show message
                    # Clear treeview
                    for item in self.teachers_tree.get_children():
                        self.teachers_tree.delete(item)
                    self.visible_teacher_ids = []
                    # Insert a message row
                    self.teachers_tree.insert("", tk.END, values=("", "No duplicate teacher names found", ""), tags=('evenrow',))
                    return
    ...
    def print_multiple_timetables(self):
        try:
            ...
            dlg.geometry("+{}+{}".format(
                ...
                self.winfo_rooty() + self.winfo_height()//2 - 250
            ))
            frame = ttk.Frame(dlg)
            frame.pack(side='top', padx=10, pady=(10,0), fill='both', expand=True)
            scrollbar = ttk.Scrollbar(frame, orient='vertical')
            scrollbar.pack(side='right', fill='y')
            lb = tk.Listbox(frame, selectmode='multiple', width=60, height=20, yscrollcommand=scrollbar.set)
            lb.pack(side='left', fill='both', expand=True)
            lb.config(font=("Segoe UI", 12))
            ...
            left_spacer = ttk.Frame(inner_frame)
            left_spacer.pack(side='left', expand=True)
            ttk.Button(inner_frame, text='Print', command=on_ok).pack(side='left', padx=8)
            ttk.Button(inner_frame, text='Cancel', command=lambda: dlg.destroy()).pack(side='left', padx=8)
            right_spacer = ttk.Frame(inner_frame)
