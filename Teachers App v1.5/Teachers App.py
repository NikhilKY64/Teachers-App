...
class TeacherTimetableApp(tk.Tk):
    ...
    def create_menu_bar(self):
        ...
        # Use a slightly larger font for the app menu for better readability
        ...
        file_menu.add_command(label="Export Teachers to Excel", command=self.export_teachers_to_excel)
        file_menu.add_command(label="Export Timetable to Excel", command=self.export_timetable_to_excel)
        file_menu.add_separator()
        ...
        settings_menu.add_command(label="Edit Period Timings", command=self.edit_period_timings_dialog)
        settings_menu.add_command(label="Manage Colors", command=self.manage_colors_dialog)
        settings_menu.add_command(label="Change Blink Interval", command=self.change_blink_interval_dialog)
        settings_menu.add_separator()
        settings_menu.add_command(label="About", command=self.show_about_dialog)
        
        # Layout Menu (separate top-level menu for quick access)
        layout_menu = tk.Menu(menubar, tearoff=0, font=menu_font)
        menubar.add_cascade(label="Layout", menu=layout_menu)
        layout_menu.add_command(label="Toggle Layout Mode (Old/New)", command=self.toggle_layout_mode)
        
        # Actions Menu (moved toolbar actions here)
        actions_menu = tk.Menu(menubar, tearoff=0, font=menu_font)
        menubar.add_cascade(label="Actions", menu=actions_menu)
        actions_menu.add_command(label="Add Teacher", command=self.add_teacher_dialog)
        actions_menu.add_command(label="Edit Teacher", command=self.edit_teacher_dialog)
        actions_menu.add_command(label="Delete Teacher", command=self.toggle_bulk_delete_mode)
        actions_menu.add_separator()
        actions_menu.add_command(label="Add Period", command=self.add_period_dialog)
        actions_menu.add_command(label="Edit Period", command=self.edit_period_dialog)
        actions_menu.add_command(label="Delete Period", command=self.delete_period)
        actions_menu.add_separator()
        actions_menu.add_command(label="Edit Period Timings", command=self.edit_period_timings_dialog)
        actions_menu.add_separator()
    ...
    def create_basic_layout(self):
        ...
        self.teacher_btn_frame.pack(fill=tk.X, pady=(10, 0), padx=6)
        ...
        self.import_btn_frame.pack(fill=tk.X, pady=(6, 6), padx=6)
    ...
    def finish_setup(self):
        ...
        self.timetable_btn_frame.pack(fill=tk.X, pady=(10, 6), padx=6)
    ...
    def show_main_buttons(self):
        ...
        try:
            # Show teacher button frames
            self.teacher_btn_frame.pack(fill=tk.X, pady=(10, 0), padx=6)
            self.import_btn_frame.pack(fill=tk.X, pady=(6, 6), padx=6)
            
            # Pack teacher buttons horizontally
    ...
    def edit_period_timings_dialog(self):
        ...
        ttk.Label(main, text="Period", font=("Segoe UI", 10, "bold")).grid(row=0, column=0, sticky="w", padx=5, pady=8)
        ttk.Label(main, text="Start Time", font=("Segoe UI", 10, "bold")).grid(row=0, column=1, sticky="w", padx=5, pady=8)
        ttk.Label(main, text="End Time", font=("Segoe UI", 10, "bold")).grid(row=0, column=2, sticky="w", padx=5, pady=8)

        entries = {}
        for i, pnum in enumerate(sorted(self.periods), start=1):
            ttk.Label(main, text=f"Period {pnum}", font=("Segoe UI", 10)).grid(row=i, column=0, sticky="w", padx=5, pady=6)
            ...
            s_entry = ttk.Entry(main, width=12, font=("Segoe UI", 10))
            s_entry.insert(0, start_str)
            s_entry.grid(row=i, column=1, sticky="w", padx=5, pady=6)
            
            e_entry = ttk.Entry(main, width=12, font=("Segoe UI", 10))
            e_entry.insert(0, end_str)
            e_entry.grid(row=i, column=2, sticky="w", padx=5, pady=6)
            
            entries[pnum] = (s_entry, e_entry)

        button_frame = ttk.Frame(main)
        button_frame.grid(row=len(self.periods)+2, column=0, columnspan=3, pady=15)
    ...
    def manage_colors_dialog(self):
        ...
        ttk.Label(main_frame, text="Current Color Settings", font=("Segoe UI", 12, "bold")).pack(anchor="w", pady=(0, 15))
        
        # Blinking color
        ttk.Label(main_frame, text=f"Blinking Color: {self.blink_color}").pack(anchor="w", pady=5)
        tk.Button(main_frame, text="Change Blinking Color",
              command=lambda: self.choose_color("blink_color", "Blinking Color"),
              bg=self.color_edit, fg=self.text_color, font=("Segoe UI", 10), activebackground="#1D4ED8", cursor="hand2").pack(anchor="w", padx=20, pady=2)

        # Muted text color
        ttk.Label(main_frame, text=f"Text Color: {self.timetable_text_muted_color}").pack(anchor="w", pady=(15, 5))
        tk.Button(main_frame, text="Change Text Color",
              command=lambda: self.choose_color("timetable_text_muted_color", "Text Color"),
              bg=self.color_edit, fg=self.text_color, font=("Segoe UI", 10), activebackground="#1D4ED8", cursor="hand2").pack(anchor="w", padx=20, pady=2)

        tk.Button(main_frame, text="Reset to Defaults", command=self.reset_colors,
              bg="#6B7280", fg=self.text_color, font=("Segoe UI", 10), activebackground="#4B5563", cursor="hand2").pack(anchor="w", padx=20, pady=(15, 5))

        tk.Button(main_frame, text="Close", command=dialog.destroy,
              bg="#6B7280", fg=self.text_color, font=("Segoe UI", 10), activebackground="#4B5563", cursor="hand2").pack(anchor="e", pady=10)
    ...
    def choose_color(self, attr_name, color_label):
    ...
    def change_blink_interval_dialog(self):
        ...
        ttk.Label(main_frame, text="Blink Interval (milliseconds)", font=("Segoe UI", 11, "bold")).pack(anchor="w", pady=(0, 10))
        ttk.Label(main_frame, text=f"Current: {self.blink_interval}ms", font=("Segoe UI", 10)).pack(anchor="w", pady=(0, 10))
        
        ttk.Label(main_frame, text="New Interval:").pack(anchor="w", pady=5)
...
• Right-click cell - Show context menu
• Left-click teacher - Select teacher
• Left-click row - Toggle select checkbox (bulk delete mode)

Menu Shortcuts:
• File → Import Teachers
• File → Import Timetable
• File → Exit
• Settings → Edit Period Timings
• Settings → Manage Colors
• Settings → Change Blink Interval
• Settings → About
• Help → How to Use
